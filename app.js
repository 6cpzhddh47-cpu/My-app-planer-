...
async function loadAllData() {
  ...
  try {
    main.innerHTML = '<div class="loading">Загружаем план...</div>';
    ...
    if (text.trim().startsWith('<') || text.includes('<!DOCTYPE')) {
      main.innerHTML = `
        <div class="loading">
          ...
          Открой Google Sheets → Поделиться → Все у кого есть ссылка → Читатель
        </div>`;
      return;
    }
...
function renderDay(container) {
  ...
  const todayTasks = state.allPlans.filter(r => toISODate(r.plan_date) === today);
  ...
  if (todayTasks.length === 0) {
    ...
    container.innerHTML = `
      <div class="create-plan-wrap">
        ...
        <div class="create-hint" style="font-size:12px">Первое нажатие создаёт план, второе загружает его</div>
      </div>`;
  ...
  state.tasks = todayTasks.map(r => ({
    ...r,
    status: r.plan_status || 'planned'
  }));
    ...
    <div class="modal">
      ...
      <div class="form-group">
        ...
        <div class="new-subtype-row">
          <input class="new-subtype-input" id="new-subtype-input" placeholder="Создать subtype..." />
...
async function saveTask() {
  ...
  try {
    ...
    console.error('Add task webhook error:', e);
  }
}
...
async function loadStats() {
  ...
  try {
    const url = `https://docs.google.com/spreadsheets/d/${SHEETS_ID}/gviz/tq?tqx=out:csv&sheet=Stats&cachebust=${Date.now()}`;
    const res = await fetch(url, { cache: 'no-store' });
    const text = await res.text();
    ...
    el.innerHTML = '<div class="loading">Ошибка загрузки статистики.</div>';
  }
}
